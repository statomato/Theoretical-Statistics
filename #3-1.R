...
g = function(u){exp(-u-exp(-u))}

fx = function(data,par){
  fx.a = par[1]
  fx.b = par[2]
  fx.sigma = par[3]
  u = with(data, max.rainfall.cm-fx.a-fx.b*t4)
  loss = with(data, sum(log(g(u))/fx.sigma-log(fx.sigma)))
  return(loss)
}

MLE = optim(0.03795566,fx,data=p4)
...
p1 <- read.csv("C:/Users/LEEEUNJIN/Desktop/2018-1/이론통계학1/3주차/part1.csv")
colnames(p1)
...
XX = -p1$rt-0.04
mean = -mean(XX[-1]); var = var(XX[-1])
alpha_0 = 1- pnorm(-mean/sqrt(var))
alpha_1 = alpha_1b/alpha_0
VA = 0.04 + 0.0220485*(alpha_1^(-0.220485)-1)/0.220485




exess = p1[p1$rt < -0.04,]
pr_exess = rank(exess$rt)/(length(exess$rt)+1)
X = -exess$rt - 0.04
exess = cbind(exess,X)
X_rank = rank(X)/(length(X)+1)



par(mfrow=c(1,3))
plot(-log(1-X_rank),log(1+X/0.01)) 
plot(-log(1-X_rank),log(1+X/0.05))
plot(-log(1-X_rank),log(1+X/0.1))


xx = -log(1-X_rank); yy = log(1+X/0.1)
